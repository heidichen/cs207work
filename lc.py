...
def lc_reader(filename):
    ...
    with open(filename) as fp:
        for num, line in enumerate(fp):
            if num == 0:
                names = [name for name in line[1:].strip().split()]
            elif num == 1:
                ## I see "B" as one of the values so it doesn't look like
                ## facet value has to be numerical
                values = [val for val in line[1:].strip().split()]
            elif line.find('#')!=0:
                lclist.append([float(f) for f in line.strip().split()])
        facet_dict = {name:val for name,val in zip(names, values)}
...
class LightCurveDB:
    ...
    def retrieve(self, facet, value):
        if facet == 'field':
            return self._filed_index[value]
        elif facet == 'tile':
            return self._tile_index[value]
        elif facet == 'color':
            return self._color_index[value]
        else:
            print ("No information on this facet")
